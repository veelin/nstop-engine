...
import com.didiglobal.turbo.engine.database.adapter.ProcessInstanceRepositoryAdapter;
...
public class FlowExecutor extends RuntimeExecutor {
    ...
    private void preExecute(RuntimeContext runtimeContext) throws ProcessException {
        ...
        String instanceDataId = saveInstanceData(runtimeContext, flowInstancePO, runtimeContext.getInstanceDataMap());
    ...
    private FlowInstancePO saveFlowInstance(RuntimeContext runtimeContext) throws ProcessException {
        ...
        ProcessInstanceRepository processInstanceRepository = processInstanceRepositoryAdapter.find(runtimeContext);
        int result = processInstanceRepository.insert(flowInstancePO);
        if (result == 1) {
    ...
    private void postExecute(RuntimeContext runtimeContext) throws ProcessException {
        ...
        if (isCompleted(runtimeContext)) {
            ProcessInstanceRepository processInstanceRepository = processInstanceRepositoryAdapter.find(runtimeContext);
            processInstanceRepository.updateStatus(runtimeContext.getFlowInstanceId(), FlowInstanceStatus.COMPLETED);
    ...
    private void preRollback(RuntimeContext runtimeContext) throws ProcessException {
        ...
        NodeInstancePO rollbackNodeInstancePO = getActiveUserTaskForRollback(runtimeContext, flowInstanceId, suspendNodeInstanceId,
    ...
    private NodeInstancePO getActiveUserTaskForRollback(RuntimeContext runtimeContext, String flowInstanceId, String suspendNodeInstanceId,
                                                        Map<String, FlowElement> flowElementMap) {
        NodeInstanceRepository nodeInstanceRepository = nodeInstanceRepositoryAdapter.find(runtimeContext);
        List<NodeInstancePO> nodeInstancePOList = nodeInstanceRepository.selectDescByFlowInstanceId(flowInstanceId);
    ...
    private void saveNodeInstanceList(RuntimeContext runtimeContext, int nodeInstanceType) {
        ...
        NodeInstanceLogRepository nodeInstanceLogRepository = nodeInstanceLogRepositoryAdapter.find(runtimeContext);
